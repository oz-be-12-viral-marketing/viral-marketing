...
from apps.accounts.choices import ACCOUNT_TYPE, BANK_CODES, CURRENCIES  # Import choices
from apps.accounts.models import Account  # Import Account model
from apps.transaction_history.choices import TransactionMethod, TransactionType  # Import choices
...
class AccountForm(forms.ModelForm):
    ...
    account_number = forms.CharField(
        widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "계좌번호"}), label="계좌번호"
    )
    account_type = forms.ChoiceField(
        choices=ACCOUNT_TYPE, widget=forms.Select(attrs={"class": "form-control"}), label="유형"
    )
...
class TransactionForm(forms.ModelForm):
    ...
    transaction_type = forms.ChoiceField(
        choices=TransactionType.choices, widget=forms.Select(attrs={"class": "form-select"}), label="유형"
    )
    transaction_method = forms.ChoiceField(
        choices=TransactionMethod.choices, widget=forms.Select(attrs={"class": "form-control"}), label="거래 방식"
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "transaction_detail": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "상세 내용 (선택 사항)"}
            ),
        }
        labels = {
            "amount": "금액",
