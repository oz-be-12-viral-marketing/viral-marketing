...
def transactions_list_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print(
                f"DEBUG: Before update - Account ID: {account.id}, Balance: {account.balance}, Transaction Type: {transaction.transaction_type}, Amount: {transaction.amount}"
            )  # DEBUG
    ...
    sentiment_analysis_subquery = SentimentAnalysis.objects.filter(transaction=OuterRef("pk")).values("pk")[:1]
    transaction_list = (
        TransactionHistory.objects.filter(account__user=request.user)
        .annotate(analysis_id=Subquery(sentiment_analysis_subquery))
        .order_by("-created_at")
    )
...
def analysis_history_view(request):
    analysis_list = SentimentAnalysis.objects.filter(transaction__account__user=request.user).order_by(
...
def generate_reports_view(request):
    ...
    weekly_report = (
        SpendingReport.objects.filter(user=request.user, report_type="weekly")  # Filter by current user
    ...
    monthly_report = (
        SpendingReport.objects.filter(user=request.user, report_type="monthly")  # Filter by current user
        .order_by("-generated_date", "-created_at")
        .first()
    )
